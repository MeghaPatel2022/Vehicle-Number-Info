...
public class zzc extends zzi implements OnAudioFocusChangeListener, OnBufferingUpdateListener, OnCompletionListener, OnErrorListener, OnInfoListener, OnPreparedListener, OnVideoSizeChangedListener, SurfaceTextureListener {
    ...
    private void zzel() {
        zzb.m20v("AdVideoView init MediaPlayer");
    ...
    private void zzem() {
        if (zzep() && this.zzAf.getCurrentPosition() > 0 && this.zzAe != 3) {
            zzb.m20v("AdVideoView nudging MediaPlayer");
            zzb(0.0f);
            this.zzAf.start();
    ...
    private void zzeo() {
        zzb.m20v("AdVideoView abandon audio focus");
        AudioManager zzev = zzev();
        if (zzev != null && this.zzAo) {
    ...
    private void zzes() {
        zzb.m20v("AdVideoView audio focus gained");
        this.zzAo = true;
        zzeu();
    }

    private void zzet() {
        zzb.m20v("AdVideoView audio focus lost");
        this.zzAo = false;
        zzeu();
    }
    ...
    public void onCompletion(MediaPlayer mp) {
        zzb.m20v("AdVideoView completion");
        zzt(5);
        zzu(5);
    ...
    public boolean onInfo(MediaPlayer mp, int what, int extra) {
        ...
        zzb.m20v("AdVideoView MediaPlayer info: " + ((String) zzAb.get(Integer.valueOf(what))) + ":" + str);
        return true;
    }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.zzAh > 0 && this.zzAi > 0) {
            ...
            if (mode == 1073741824 && mode2 == 1073741824) {
                ...
                int i = (this.zzAi * size) / this.zzAh;
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        zzb.m20v("AdVideoView surface created");
        zzel();
        zzhu.zzHK.post(new Runnable() {
            public void run() {
                if (zzc.this.zzAq != null) {
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        zzb.m20v("AdVideoView surface destroyed");
    ...
    public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int w, int h) {
        boolean z = true;
        zzb.m20v("AdVideoView surface changed");
        boolean z2 = this.zzAe == 3;
        if (!(this.zzAh == w && this.zzAi == h)) {
            z = false;
        }
        if (this.zzAf != null && z2 && z) {
            if (this.zzAp != 0) {
                seekTo(this.zzAp);
            }
            play();
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        zzb.m20v("AdVideoView size changed: " + width + " x " + height);
    ...
    public void seekTo(int millis) {
        zzb.m20v("AdVideoView seek " + millis);
        if (zzep()) {
