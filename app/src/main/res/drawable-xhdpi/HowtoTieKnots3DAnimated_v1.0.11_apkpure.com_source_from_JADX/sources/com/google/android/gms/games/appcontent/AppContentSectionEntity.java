...
public final class AppContentSectionEntity implements SafeParcelable, AppContentSection {
    ...
    public AppContentSectionEntity(AppContentSection section) {
        ...
        List zztv = section.zztv();
        int size2 = zztv.size();
        this.zzasx = new ArrayList<>(size2);
        for (int i2 = 0; i2 < size2; i2++) {
            this.zzasx.add((AppContentCardEntity) ((AppContentCard) zztv.get(i2)).freeze());
        }
        List zztl = section.zztl();
        int size3 = zztl.size();
        this.zzaso = new ArrayList<>(size3);
        for (int i3 = 0; i3 < size3; i3++) {
            this.zzaso.add((AppContentAnnotationEntity) ((AppContentAnnotation) zztl.get(i3)).freeze());
        }
    }
