...
public class Step extends Activity implements OnClickListener, OnTouchListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.f74db = dbOpenHelper.getReadableDatabase();
        selectItem();
        selectSteps();
        selectFrames();
        this.myCursor.close();
        dbOpenHelper.close();
        this.preview_button = (ImageButton) findViewById(C1295R.C1297id.steps_counter);
        this.next_button = (ImageButton) findViewById(C1295R.C1297id.replay);
        this.replay_button = (ImageButton) findViewById(C1295R.C1297id.preview);
        this.counter = (TextView) findViewById(C1295R.C1297id.extra_tech_img);
        this.title = (TextView) findViewById(C1295R.C1297id.title);
        this.bgImage = (ImageView) findViewById(C1295R.C1297id.img);
        this.itemExtraTechImage = (ImageView) findViewById(C1295R.C1297id.twitter_logo);
        this.techImage = (ImageView) findViewById(C1295R.C1297id.item_extra_tech_img);
        this.extraTechImage = (ImageView) findViewById(C1295R.C1297id.tech_img);
    ...
    private void selectItem() {
        this.myCursor = this.f74db.rawQuery("SELECT id, ident, name, is_base FROM lesson_item WHERE is_active=1 AND ident=" + this.ident, null);
        this.myCursor.moveToFirst();
        this.item = new Item();
        this.item.name = this.myCursor.getString(this.myCursor.getColumnIndex("name")).replace("&amp;", "&").replace("&#39;", "'");
        this.item.f71id = this.myCursor.getInt(this.myCursor.getColumnIndex("id"));
    ...
    private void selectSteps() {
        this.myCursor = this.f74db.rawQuery("SELECT id, step_number, description, sort_order, frames_count FROM lesson_step WHERE item_id=" + this.item.f71id + " ORDER BY sort_order ASC", null);
    ...
    private void selectFrames() {
        this.myCursor = this.f74db.rawQuery("SELECT sort_order, step_id FROM lesson_frame WHERE item_id=" + this.item.f71id + " ORDER BY sort_order ASC", null);
        while (this.myCursor.moveToNext()) {
