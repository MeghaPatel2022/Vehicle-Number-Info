...
class SpinnerCompat extends AbsSpinnerCompat implements OnClickListener {
    ...
    SpinnerCompat(Context context, int mode) {
        this(context, null, C0142R.attr.spinnerStyle, mode);
    }
    ...
    SpinnerCompat(Context context, AttributeSet attrs, int defStyle, int mode) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0142R.styleable.Spinner, defStyle, 0);
        if (a.hasValue(C0142R.styleable.Spinner_android_background)) {
            setBackgroundDrawable(a.getDrawable(C0142R.styleable.Spinner_android_background));
        }
        if (mode == -1) {
            mode = a.getInt(C0142R.styleable.Spinner_spinnerMode, 0);
        }
        switch (mode) {
            case 0:
            ...
            case 1:
                ...
                this.mDropDownWidth = a.getLayoutDimension(C0142R.styleable.Spinner_android_dropDownWidth, -2);
                popup.setBackgroundDrawable(a.getDrawable(C0142R.styleable.Spinner_android_popupBackground));
                this.mPopup = popup;
        ...
        this.mGravity = a.getInt(C0142R.styleable.Spinner_android_gravity, 17);
        this.mPopup.setPromptText(a.getString(C0142R.styleable.Spinner_prompt));
        this.mDisableChildrenWhenDisabled = a.getBoolean(C0142R.styleable.Spinner_disableChildrenWhenDisabled, false);
        a.recycle();
